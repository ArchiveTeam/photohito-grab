...
LUA_SHA1 = get_hash(os.path.join(CWD, 'photohito.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'photo':
                wget_args.extend(['--warc-header', 'photohito-photo: '+item_value])
                wget_args.append('https://photohito.com/photo/'+item_value+'/')
            elif item_type == 'user':
                wget_args.extend(['--warc-header', 'photohito-user: '+item_value])
                wget_args.append('https://photohito.com/user/'+item_value+'/')
            elif item_type == 'gallery':
                wget_args.extend(['--warc-header', 'photohito-gallery: '+item_value])
                wget_args.append('https://photohito.com/user/gallery/'+item_value+'/')
            elif item_type == 'media':
                wget_args.extend(['--warc-header', 'photohito-media: '+item_value])
                wget_args.append('https://'+item_value)
            elif item_type == 'page':
                wget_args.extend(['--warc-header', 'photohito-page: '+item_value])
                wget_args.append('https://'+item_value)
            else:
...
project = Project(
    title=TRACKER_ID,
    project_html='''
        <img class="project-logo" alt="Project logo" src="https://wiki.archiveteam.org/images/6/61/Photohito-icon.png" height="50px" title=""/>
        <h2>PHOTOHITO <span class="links"><a href="https://photohito.com/">Website</a> &middot; <a href="https://tracker.archiveteam.org/photohito/">Leaderboard</a> &middot; <a href="https://wiki.archiveteam.org/index.php/Photohito">Wiki</a></span></h2>
        <p>Archiving PHOTOHITO（フォトヒト）.</p>
    ''',
